...
class SolverEMChampagne(BaseSolver):
    ...
    def make_inverse_operator(self, forward, evoked, *args, alpha='auto', 
        ...
        inverse_operators = []
        for alpha in self.alphas:
            inverse_operator = self.em_champagne(evoked.data, alpha, max_iter=max_iter, prune=prune, pruning_thresh=pruning_thresh, convergence_criterion=convergence_criterion)
            inverse_operators.append( inverse_operator )
        self.inverse_operators = [InverseOperator(inverse_operator, self.name) for inverse_operator in inverse_operators]
        return self
...
class SolverTEMChampagne(BaseSolver):
    ...
    def make_inverse_operator(self, forward, evoked, *args, alpha='auto', 
        ...
        inverse_operators = []
        for alpha in self.alphas:
            inverse_operator = self.tem_champagne(evoked.data, alpha, max_iter=max_iter, prune=prune, pruning_thresh=pruning_thresh, convergence_criterion=convergence_criterion, theta=theta)
            inverse_operators.append( inverse_operator )
        self.inverse_operators = [InverseOperator(inverse_operator, self.name) for inverse_operator in inverse_operators]
        return self
...
class SolverLowSNRChampagne(BaseSolver):
    ...
    def make_inverse_operator(self, forward, evoked, *args, alpha='auto', 
        ...
        inverse_operators = []
        for alpha in self.alphas:
            inverse_operator = self.low_snr_champagne(evoked.data, alpha, max_iter=max_iter, prune=prune, pruning_thresh=pruning_thresh, convergence_criterion=convergence_criterion)
            inverse_operators.append( inverse_operator )
        self.inverse_operators = [InverseOperator(inverse_operator, self.name) for inverse_operator in inverse_operators]
        return self
